...
import { AlertCircle } from "lucide-react"; //CheckCircle, XCircle
...
const Quiz: React.FC = () => {
  ...
  const fetchAllQuestions = useCallback(async () => {
    try {
      ...
      setSubjectCategories([
        ...new Set(allQuestions.map((q) => q.subject_category_name)),
      ]);
    ...
    if (currentPage !== pageIndex) {
      ...
      setTimeout(() => {
        const questionElement =
          questionRefs.current[questions[questionIndex].id];
        if (questionElement) {
  ...
  const isAnswerCorrect = (questionId: number) => {
    ...
    const correctChoice = choices[questionId]?.find(
  ...
  return (
    <div className="flex flex-col h-screen bg-background dark:bg-background-dark text-text dark:text-text-dark">
      <div className="p-4 border-b">
          ...
          <div className="flex items-center gap-2">
            <label className="inline-flex items-center cursor-pointer">
              ...
              <span className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                {sidebarOpen ? "Questions is Open" : "Qeustions is Closed"}
              </span>
            </label>
            <Timer
        ...
        <Progress
          value={((QUIZ_DURATION - timeRemaining) / QUIZ_DURATION) * 100}
          className="h-2"
        />
      </div>
                  ...
                  <div className="space-y-2">
                    {choices[question.id]?.map((choice) => (
                      <div
                        key={choice.id}
                        onClick={() =>
                          handleChoiceSelect(question.id, choice.id)
                        }
