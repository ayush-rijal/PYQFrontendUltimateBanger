...
// import { useGetCategoryChildrenQuery } from "@/redux/features/quizApiSlice";
...
//   const { data: categories, isLoading, error } = useGetCategoryChildrenQuery(currentPath);
...
//   const CategoryCard = ({ title, variant, categoryPath }: CategoryCardProps) => (
//     <Link href={`/quiz-app/category/${categoryPath}/${title}`}>
//       <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
//         <Card className={`h-full flex flex-col bg-white dark:bg-gray-800 border ${variantStyles[variant]} rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer overflow-hidden`}>
//           <CardHeader className="pb-2">
//             <CardTitle className="text-xl font-semibold text-gray-900 dark:text-gray-100 line-clamp-1">{title}</CardTitle>
//           </CardHeader>
//           <CardContent className="flex-1 flex flex-col justify-between pt-0">
//             <div>
//               <p className="text-sm text-gray-600 dark:text-gray-400 mb-4 line-clamp-2">Explore this category and test your knowledge!</p>
//               <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400">
//                 <div className="flex items-center"><BookOpen className="w-4 h-4 mr-1.5" /><span>10 Questions</span></div>
//                 <div className="flex items-center"><Clock className="w-4 h-4 mr-1.5" /><span>15 mins</span></div>
//               </div>
//             </div>
...
//                 <PlayCircle className="w-5 h-5 mr-2 group-hover:scale-105 transition-transform" /> Explore Category
//               </Button>
//             </motion.div>
//           </CardContent>
//         </Card>
...
//               >
//                 <CategoryCard title={cat.title} variant={cat.variant} categoryPath={cat.categoryPath} />
//               </motion.div>
//             ))
//           ) : (
//             <p className="text-center text-gray-500 dark:text-gray-400 col-span-full">
//               No subcategories found for "{searchTerm || currentPath.split("/").pop()}"
//             </p>
//           )}
//         </motion.div>
//       </div>
...
"use client";
import { useState } from "react";
import { useGetCategoryChildrenQuery,useGetQuizFilesQuery } from "@/redux/features/quizApiSlice";
import Link from "next/link";
import { useParams } from "next/navigation";
import { motion } from "framer-motion";
import { Button } from "@/components/ui/button";
...
export default function CategoryPage() {
  ...
  const { data: questionFiles, isLoading: isFetchingFiles, error: fetchFilesError } = useGetQuizFilesQuery(currentPath, {
    ...
    <Link href={`/quiz-app/quiz/${categoryPath}/${title}`}>
      <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
        <Card className="h-full flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer overflow-hidden">
          ...
          <CardContent className="flex-1 flex flex-col justify-between pt-0">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4 line-clamp-2">Start this quiz to test your knowledge!</p>
              <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400">
                <div className="flex items-center"><BookOpen className="w-4 h-4 mr-1.5" /><span>10 Questions</span></div>
                <div className="flex items-center"><Clock className="w-4 h-4 mr-1.5" /><span>15 mins</span></div>
              </div>
            ...
            <motion.div className="mt-6" whileHover={{ scale: 1.03 }}>
              <Button className="w-full bg-green-600 hover:bg-green-700 dark:bg-green-500 dark:hover:bg-green-600 text-white font-medium rounded-lg py-2.5 transition-colors duration-200 flex items-center justify-center">
                <PlayCircle className="w-5 h-5 mr-2 group-hover:scale-105 transition-transform" /> Start Quiz
              </Button>
            </motion.div>
          </CardContent>
        </Card>
          ...
          >
            {isFetchingFiles ? (
              ...
              <div className="col-span-full text-center text-red-500">
                Error: {fetchFilesError instanceof Error ? fetchFilesError.message : "An unknown error occurred"}
              </div>
              ...
              filteredQuestionFiles.map((file, index) => (
                <motion.div
                  ...
                  transition={{ duration: 0.3, delay: index * 0.1 }}
                >
                  <QuestionFileCard title={file.title} categoryPath={currentPath} />
                </motion.div>
              ))
            ) : (
              <p className="text-center text-gray-500 dark:text-gray-400 col-span-full">
                No quizzes found for "{searchTerm || currentPath.split("/").pop()}"
              </p>
            )}
          </motion.div>
        ) : (
